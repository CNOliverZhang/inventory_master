...
const index: React.FC = () => {
  ...
  useEffect(() => {
    ...
    request.get(api.richTextEditor.version.list).then((versionListRes) => {
      if (versionListRes.status === 200) {
        setVersionList(versionListRes.data.list);
        if (user?.isAdmin) {
          request.get(api.richTextEditor.client.count).then((versionClientCountRes) => {
            loading.close();
  ...
  const addVersion = (values: VersionForm) => {
    ...
    request.post(api.richTextEditor.version.add, formData, {
      timeout: 10 * 60 * 1000,
    }).then((updateRes) => {
      loading.close();
      if (updateRes.status === 200) {
        ...
        request.get(api.richTextEditor.version.list).then((getListRes) => {
