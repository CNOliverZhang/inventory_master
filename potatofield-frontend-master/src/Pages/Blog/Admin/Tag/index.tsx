...
import { BlogArticleTag, BlogArticleTagForm } from '@/Pages/Blog/types';

const index: React.FC = () => {
  const [tagList, setTagList] = useState<BlogArticleTag[]>([]);
  const [labelSubmitting, setLabelSubmitting] = useState(false);
  const mediaFileTagForm = useForm<BlogArticleTagForm>();
  ...
  const addTag = (tagForm: BlogArticleTagForm) => {
    ...
    request.post(api.blog.tag.add, tagForm).then((res) => {
  ...
  const removeTag = (tag: BlogArticleTag) => {
    new Dialog({
      title: '操作确认',
      content: `是否确认删除标签“${tag.name}”？`,
      onConfirm: () => {
        const loading = new Loading();
        request.post(api.blog.tag.remove, {
          id: tag.id,
        }).then((res) => {
          loading.close();
          if (res.status === 200) {
