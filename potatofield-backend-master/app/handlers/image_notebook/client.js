...
module.exports = {
  register: async (ctx) => {
    ...
    const { ImageNotebook_client, ImageNotebook_version, ImageNotebook_usage } = ctx.db;
    const { identifier, version, platform } = ctx.params;
    try {
      let client = await ImageNotebook_client.findOne({
        where: { identifier },
      });
      const clientVersion = await ImageNotebook_version.findOne({
        where: { code: version },
      });
      if (!clientVersion) {
      ...
      if (client) {
        ...
        client = await ImageNotebook_client.create({
          identifier,
      ...
      await ImageNotebook_usage.create({
        time: now,
        clientId: client.id,
      });
      ctx.body = {
  ...
  countNew: async (ctx) => {
    ...
    const { ImageNotebook_client } = ctx.db;
    ...
    const data = (await ImageNotebook_client.findAll({
      where: {
        registerTime: {
  ...
  count: async (ctx) => {
    const { ImageNotebook_client, ImageNotebook_version } = ctx.db;
    const { count } = (await ImageNotebook_client.findOne({
      attributes: [
        [Sequelize.fn('count', '*'), 'count'],
      ],
    })).toJSON();
    const versionList = (await ImageNotebook_version.findAll()).map((item) => item.toJSON());
    const versionObject = {};
    versionList.forEach((item) => {
      versionObject[item.id] = item.code;
    });
    const versionsRawData = (await ImageNotebook_client.findAll({
      attributes: [
        [Sequelize.fn('count', '*'), 'count'],
        'versionId',
      ],
    ...
    const platforms = await ImageNotebook_client.findAll({
      attributes: [
        [Sequelize.fn('count', '*'), 'count'],
        'platform',
      ],
