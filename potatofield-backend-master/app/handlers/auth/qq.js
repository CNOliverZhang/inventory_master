...
const qqWebsiteConfig = require('../../config/qq_website');
...
module.exports = {
  login: async (ctx) => {
    ...
    const { code, redirectUri } = ctx.params;
    try {
      const accessTokenRes = await axios.get(
        `${api.qq.getUserAccessToken}?client_id=${qqWebsiteConfig.appid}&client_secret=${qqWebsiteConfig.secret}&code=${code}&redirect_uri=${redirectUri}&grant_type=authorization_code`,
      );
      ...
      const unionIdRes = await axios.get(
        `${api.qq.getUnionId}?access_token=${accessToken}&fmt=json&unionid=1`,
      );
      if (unionIdRes?.status !== 200) {
        throw new Error('get user union id error');
      }
      const openId = unionIdRes?.data?.openid;
      const unionId = unionIdRes?.data?.unionid;
  ...
  register: async (ctx) => {
    ...
    try {
      ...
      const userInfoRes = await axios.get(
        `${api.qq.getUserInfo}?access_token=${accessToken}&oauth_consumer_key=${qqWebsiteConfig.appid}&openid=${openId}`,
      );
      ...
      if (credentialInstance) {
        throw new Error('qq already bound to another user');
      }
      const newUser = await Auth_user.create({ isAdmin: false });
      const userId = newUser.id;
      const avatarUrl = userInfoRes?.data?.figureurl_qq
        || userInfoRes?.data?.figureurl_qq_2
        || userInfoRes?.data?.figureurl_qq_1;
  ...
  loginAndConnect: async (ctx) => {
    ...
    try {
      ...
      const qqCredentialInstance = await Auth_credential.findOne({
        where: {
          identifier: unionId,
        },
      });
      if (qqCredentialInstance) {
        throw new Error('qq already bound to another user');
      }
      await Auth_credential.create({
        userId,
      ...
      ctx.throw(403, `Login and bind qq error: ${err.message}.`);
    }
  },
};
